...
public class UserDao {
    ...
    public static int validateUser(String email, String password) throws SQLException {
        String checkEmail = null;
        String checkPassword = null;
        int id = 0;

        Connection connection = DbUtils.connectToDb();
        PreparedStatement psCheckEmailAndPassword = connection.prepareStatement("SELECT id, email, password FROM users WHERE email = ?");
        ...
        while (rsCheckEmailAndPassword.next()) {
            id = rsCheckEmailAndPassword.getInt("id");
